...
```{r}
# survfit(surv_object ~ num_symptoms, data=std)

summary(coxph(surv_object ~ num_symptoms, data=std))
```

```{r}
KM_num_symp <- survfit(surv_object ~ num_symptoms, data = std)
KM_num_symp <- ggsurvplot(KM_num_symp) + labs(title = "Kaplan-Meier Curves for Number of Symptoms") 
# ggplotly(KM_num_symp[[1]])
KM_num_symp
```
