...
## KM Curves for variables in the model

```{r}
KM_iinfct <- survfit(surv_object ~ iinfct, data = std)
KM_iinfct <- ggsurvplot(KM_iinfct) + labs(title = "Kaplan-Meier Curves for Initial Infection Type") 
KM_iinfct
```

```{r}
KM_condom <- survfit(surv_object ~ condom, data = std)
KM_condom <- ggsurvplot(KM_condom) + labs(title = "Kaplan-Meier Curves for Condom Use") 
KM_condom
```


```{r}
KM_vag <- survfit(surv_object ~ vagina, data = std)
KM_vag <- ggsurvplot(KM_vag) + labs(title = "Kaplan-Meier Curves for Vagina Involvement at Exam") 
KM_vag
```

```{r}
KM_dch <- survfit(surv_object ~ dchexam, data = std)
KM_dch <- ggsurvplot(KM_dch) + labs(title = "Kaplan-Meier Curves for Discharge at Exam") 
KM_dch
```

```{r}
ggplot(data=std)+
  geom_boxplot(aes(x=as.factor(std$yschool), y=age, fill=yschool))+
  labs(title="Years of Education vs. Age", x="Years of Schooling", y="Age")
```
```{r}
ggplot(data=std)+
  geom_boxplot(aes(x=as.factor(npartner), y=age, fill=npartner))+
  labs(title="Years of Education vs. Age", x="Number of Partners", y="Age")
...
fig_yschool <-
  plot_ly(
    data = data.frame(table(std$yschool)),
    labels = ~ Var1,
    values = ~ Freq,
    type = 'pie'
  )
...
fig_race <-
  plot_ly(
    data = data.frame(table(std$race)),
    labels = ~ Var1,
    values = ~ Freq,
    type = 'pie'
  )
